create database tms_dws
    LOCATION 'hdfs://cdh01:8020/bigdata_warehouse/tms/tms_dws/';


drop table if exists tms_dws.dws_trade_org_cargo_type_order_1d;
create external table tms_dws.dws_trade_org_cargo_type_order_1d(
    ...
    stored as orc
    location '/bigdata_warehouse/tms/tms_dws/dws_trade_org_cargo_type_order_1d'
...
from (select org_id,
      ...
      from (select order_id,
            ...
            from (select order_id,
                         ...
                         amount,
                         dt
                  from tms_dwd.dwd_trade_order_detail_inc) detail
            group by order_id,
                     cargo_type,
                     cargo_type_name,
                     sender_district_id,
...
select * from tms_dws.dws_trade_org_cargo_type_order_1d;


drop table if exists tms_dws.dws_trans_org_receive_1d;
create external table tms_dws.dws_trans_org_receive_1d(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dws/dws_trans_org_receive_1d'
    tblproperties ('orc.compress'='snappy');
...
from (select order_id,
      ...
      from (select order_id,
                   ...
                   sender_district_id,
                   dt
            from tms_dwd.dwd_trans_receive_detail_inc) detail
               left join
           (select id org_id,
                   org_name,
                   region_id
...
select * from tms_dws.dws_trans_org_receive_1d;



drop table if exists tms_dws.dws_trans_dispatch_1d;
create external table tms_dws.dws_trans_dispatch_1d(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dws/dws_trans_dispatch_1d'
    tblproperties('orc.compress'='snappy');
...
select * from tms_dws.dws_trans_dispatch_1d;

drop table if exists tms_dws.dws_trans_org_truck_model_type_trans_finish_1d;
create external table tms_dws.dws_trans_org_truck_model_type_trans_finish_1d(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dws/dws_trans_org_truck_model_type_trans_finish_1d/'
    tblproperties('orc.compress'='snappy');
...
select org_id,
       ...
       sum(finish_dur_sec)as finish_dur_sec,
       dt
from (select id,
             start_org_id   org_id,
             ...
             finish_dur_sec,
             dt
      from tms_dwd.dwd_trans_trans_finish_inc) trans_finish
         left join
     (select id,
             truck_model_type,
             truck_model_type_name
      from tms_dim.dim_truck_full
      where dt = '2025-07-13') truck_info
...
select * from tms_dws.dws_trans_org_truck_model_type_trans_finish_1d;


drop table if exists tms_dws.dws_trans_org_deliver_suc_1d;
create external table tms_dws.dws_trans_org_deliver_suc_1d(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dws/dws_trans_org_deliver_suc_1d/'
    tblproperties('orc.compress'='snappy');
...
select * from tms_dws.dws_trans_org_deliver_suc_1d;


drop table if exists tms_dws.dws_trans_org_sort_1d;
create external table tms_dws.dws_trans_org_sort_1d(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dws/dws_trans_org_sort_1d/'
    tblproperties('orc.compress'='snappy');
...
from (select org_id,
     ...
     (select id,
             ...
             parent_id
      from tms_dim.dim_region_full
      where dt = '2025-07-13') province_for_level1
     ...
     (select id,
             ...
             parent_id
      from tms_dim.dim_region_full
      where dt = '2025-07-13') province_for_level2
     on province_for_level1.parent_id = province_for_level2.id;

select * from tms_dws.dws_trans_org_sort_1d;


drop table if exists tms_dws.dws_trade_org_cargo_type_order_nd;
create external table tms_dws.dws_trade_org_cargo_type_order_nd(
    ...
    partitioned by(dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dws/dws_trade_org_cargo_type_order_nd'
    tblproperties('orc.compress' = 'snappy');
...
from tms_dws.dws_trade_org_cargo_type_order_1d lateral view
...
select * from tms_dws.dws_trade_org_cargo_type_order_nd;

drop table if exists tms_dws.dws_trans_org_receive_nd;
create external table tms_dws.dws_trans_org_receive_nd(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dws/dws_trans_org_receive_nd/'
    tblproperties ('orc.compress'='snappy');
...
where dt >= date_add('2025-07-13', -recent_days + 1)
group by org_id,
         org_name,
         city_id,
         city_name,
...
select * from tms_dws.dws_trans_org_receive_nd;


drop table if exists tms_dws.dws_trans_dispatch_nd;
create external table tms_dws.dws_trans_dispatch_nd(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dws/dws_trans_dispatch_nd/'
    tblproperties('orc.compress'='snappy');
...
select recent_days,
       sum(order_count)as  order_count,
       sum(order_amount) as order_amount
from tms_dws.dws_trans_dispatch_1d lateral view
         explode(array(7, 30)) tmp as recent_days
where dt >= date_add('2025-07-13', -recent_days + 1)
group by recent_days;

select * from tms_dws.dws_trans_dispatch_nd;

drop table if exists tms_dws.dws_trans_shift_trans_finish_nd;
create external table tms_dws.dws_trans_shift_trans_finish_nd(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dws/dws_trans_shift_trans_finish_nd/'
    tblproperties('orc.compress'='snappy');
...
from (select recent_days,
     ...
     (select id,
      ...
      where dt = '2025-07-13') for_line_name
...
select * from tms_dws.dws_trans_shift_trans_finish_nd;


drop table if exists tms_dws.dws_trans_org_deliver_suc_nd;
create external table tms_dws.dws_trans_org_deliver_suc_nd(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dws/dws_trans_org_deliver_suc_nd/'
    tblproperties('orc.compress'='snappy');
...
select org_id,
       ...
       recent_days,
       sum(order_count)as order_count
from tms_dws.dws_trans_org_deliver_suc_1d lateral view
         explode(array(7, 30)) tmp as recent_days
where dt >= date_add('2025-07-13', -recent_days + 1)
group by org_id,
         org_name,
...
select * from tms_dws.dws_trans_org_deliver_suc_nd;

drop table if exists tms_dws.dws_trans_org_sort_nd;
create external table tms_dws.dws_trans_org_sort_nd(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dws/dws_trans_org_sort_nd/'
    tblproperties('orc.compress'='snappy');
...
from tms_dws.dws_trans_org_sort_1d lateral view
         explode(array(7, 30)) tmp as recent_days
group by org_id,
         org_name,
         city_id,
...
select * from tms_dws.dws_trans_org_sort_nd;
-- 物流域发单历史至今汇总表
drop table if exists tms_dws.dws_trans_dispatch_td;
create external table tms_dws.dws_trans_dispatch_td(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dws/dws_trans_dispatch_td'
    tblproperties('orc.compress'='snappy');
...
select * from tms_dws.dws_trans_dispatch_td;



drop table if exists tms_dws.dws_trans_bound_finish_td;
create external table tms_dws.dws_trans_bound_finish_td(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location 'warehouse/tms/dws/tms_dws.dws_trans_bound_finish_td'
    tblproperties('orc.compress'='snappy');
...
select * from tms_dws.dws_trans_bound_finish_td;
