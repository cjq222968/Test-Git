-- create database tms_dwd
--     LOCATION 'hdfs://cdh01:8020/bigdata_warehouse/tms/tms_dwd/';
--
-- use tms;


drop table if exists tms_dwd.dwd_trade_order_detail_inc;
create external table tms_dwd.dwd_trade_order_detail_inc(
    ...
    stored as orc
    location '/bigdata_warehouse/tms/tms_dwd/dwd_trade_order_detail_inc'
...
select * from tms_dwd.dwd_trade_order_detail_inc;


drop table if exists tms_dwd.dwd_trade_pay_suc_detail_inc;
create external table tms_dwd.dwd_trade_pay_suc_detail_inc(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dwd/dwd_trade_pay_suc_detail_inc'
    tblproperties('orc.compress' = 'snappy');
...
select * from tms_dwd.dwd_trade_pay_suc_detail_inc;



drop table if exists tms_dwd.dwd_trade_order_cancel_detail_inc;
create external table tms_dwd.dwd_trade_order_cancel_detail_inc(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dwd/dwd_trade_order_cancel_detail_inc'
    tblproperties('orc.compress' = 'snappy');
...
select * from tms_dwd.dwd_trade_order_cancel_detail_inc;




drop table if exists tms_dwd.dwd_trans_receive_detail_inc;
create external table tms_dwd.dwd_trans_receive_detail_inc(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dwd/dwd_trans_receive_detail_inc'
    tblproperties('orc.compress' = 'snappy');
...
select * from tms_dwd.dwd_trans_receive_detail_inc;



drop table if exists tms_dwd.dwd_trans_dispatch_detail_inc;
create external table tms_dwd.dwd_trans_dispatch_detail_inc(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dwd/dwd_trans_dispatch_detail_inc'
    tblproperties('orc.compress' = 'snappy');
...
select * from tms_dwd.dwd_trans_dispatch_detail_inc;



drop table if exists tms_dwd.dwd_trans_bound_finish_detail_inc;
create external table tms_dwd.dwd_trans_bound_finish_detail_inc(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dwd/dwd_trans_bound_finish_detail_inc'
    tblproperties('orc.compress' = 'snappy');
...
select * from tms_dwd.dwd_trans_bound_finish_detail_inc;
...
drop table if exists tms_dwd.dwd_trans_deliver_suc_detail_inc;
create external table tms_dwd.dwd_trans_deliver_suc_detail_inc(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dwd/dwd_trans_deliver_suc_detail_inc'
    tblproperties('orc.compress' = 'snappy');
...
from (
    ...
    where dt = '2025-07-13'
      and after.is_deleted = '0'
      and after.status not in ('60020', '60030', '60040', '60050')
) info
              on cargo.order_id = info.id

-- 修正：补全 JOIN 条件
         left join (
    select id, name from ods_base_dic
    where dt = '2025-07-13' and is_deleted = '0'
) dic_for_cargo_type on cast(cargo.cargo_type as string) = dic_for_cargo_type.id

         left join (
    select id, name from ods_base_dic
    where dt = '2025-07-13' and is_deleted = '0'
) dic_for_status on cast(info.status as string) = dic_for_status.id

         left join (
    select id, name from ods_base_dic
    where dt = '2025-07-13' and is_deleted = '0'
) dic_for_collect_type on cast(info.collect_type as string) = dic_for_collect_type.id

         left join (
    select id, name from ods_base_dic
    where dt = '2025-07-13' and is_deleted = '0'
) dic_for_payment_type on cast(info.payment_type as string) = dic_for_payment_type.id;
...
select * from tms_dwd.dwd_trans_deliver_suc_detail_inc;


-- 物流域签收事务事实表
drop table if exists tms_dwd.dwd_trans_sign_detail_inc;
create external table tms_dwd.dwd_trans_sign_detail_inc(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dwd/dwd_trans_sign_detail_inc'
    tblproperties('orc.compress' = 'snappy');
...
select * from tms_dwd.dwd_trans_sign_detail_inc;


drop table if exists tms_dwd.dwd_trade_order_process_inc;
create external table tms_dwd.dwd_trade_order_process_inc(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dwd/dwd_order_process'
    tblproperties('orc.compress' = 'snappy');
...
select * from tms_dwd.dwd_trade_order_process_inc;


drop table if exists tms_dwd.dwd_trans_trans_finish_inc;
create external table tms_dwd.dwd_trans_trans_finish_inc(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dwd/dwd_trans_trans_finish_inc'
    tblproperties('orc.compress' = 'snappy');
...
select * from tms_dwd.dwd_trans_trans_finish_inc;


drop table if exists tms_dwd.dwd_bound_inbound_inc;
create external table tms_dwd.dwd_bound_inbound_inc(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dwd/dwd_bound_inbound_inc'
    tblproperties('orc.compress' = 'snappy');
...
select * from tms_dwd.dwd_bound_inbound_inc;


drop table if exists tms_dwd.dwd_bound_sort_inc;
create external table tms_dwd.dwd_bound_sort_inc(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dwd/dwd_bound_sort_inc'
    tblproperties('orc.compress' = 'snappy');
...
select * from tms_dwd.dwd_bound_sort_inc;


drop table if exists tms_dwd.dwd_bound_outbound_inc;
create external table tms_dwd.dwd_bound_outbound_inc(
    ...
    partitioned by (dt string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/tms_dwd/dwd_bound_outbound_inc'
    tblproperties('orc.compress' = 'snappy');
...
select * from tms_dwd.dwd_bound_outbound_inc;