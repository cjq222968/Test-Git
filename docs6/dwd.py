...
import atexit
import time
from datetime import datetime  # 用于获取当天日期
...
from (select after.id,
     ...
     (select after.id,
      ...
      where dt = '{date}'
        and after.is_deleted = '0'
        and after.status in ('60040', '60050')) info
     on cargo.order_id = info.id
         left join
     (select id, name from ods_base_dic where dt = '{date}' and is_deleted = '0') dic_for_cargo_type
     on cargo.cargo_type = cast(dic_for_cargo_type.id as string)
...
select after.id,
       ...
       after.org_id,
       date_format(from_utc_timestamp(
                           to_timestamp(cast(after.outbound_time as bigint) * 1000), 'UTC'),
...
where dt = '{date}'
  ...
  and after.is_deleted = '0';
"""
check_bound_outbound = "select id, order_id, outbound_time, dt from tms_dwd.dwd_bound_outbound_inc limit 5;"

create_and_load_dwd_table(drop_bound_outbound, create_bound_outbound, insert_bound_outbound, check_bound_outbound)


# 停止SparkSession并清理资源
clean_temp_dir()  # 程序正常结束时手动清理
spark.stop()
print("所有DWD层表处理完成")