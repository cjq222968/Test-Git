...
# 获取SparkContext并设置日志级别
sc = spark.sparkContext
sc.setLogLevel("ERROR")  # 仅显示错误日志
...
# 设置Spark临时目录
spark.sparkContext.setLocalProperty("spark.local.dir", temp_dir)

def create_and_load_table(drop_sql, create_sql, insert_sql, check_sql):
    """封装表创建和数据加载的通用逻辑，实现"有数据则跳过"功能"""
    try:
        # 1. 提取表名（使用正则表达式）
...
def clean_temp_dir():
    """脚本退出时手动清理临时目录，忽略不存在的文件错误"""
    if os.path.exists(temp_dir):
        try:
            # 强制删除目录及内容，忽略错误
            shutil.rmtree(temp_dir, ignore_errors=True)
            print(f"临时目录 {temp_dir} 已手动清理")
        except Exception as e:
            print(f"清理临时目录警告: {str(e)}")  # 仅警告，不中断执行

# 注册退出钩子，确保脚本结束时清理临时目录
atexit.register(clean_temp_dir)

# 1. 显示数据库
spark.sql("show databases;").show()

# 2. 创建数据库tms_dim（存在则跳过）
spark.sql(f"""
...
select after.id,
       ...
       after.gender,
       -- 修复：将bigint转换为timestamp（乘以1000适配毫秒级）
...
# 停止SparkSession（脚本退出时会自动调用clean_temp_dir）
spark.stop()